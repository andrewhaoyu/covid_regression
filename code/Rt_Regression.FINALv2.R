...
# srun --pty -p test -t 0-04:00 --mem 10000  /bin/bash
...
OUT_TABLE_NAME = "International_Rt_Intervention.csv"
...
INTERVENTION_PARAMS <- list(
  ...
  MOB_VAR = "residential" # which mobility category to use: residential or workplace
)
...
dt_reg_mob <- dt_reg[complete.cases(dt_reg[paste0("mobility_delta_", INTERVENTION_PARAMS$MOB_VAR)]), ]
...
eqn = add_formula(fe_eqn, paste0("mobility_delta_", INTERVENTION_PARAMS$MOB_VAR), TRUE)
...
if (OUTPUT_OPTION$INTERVENTION_ONLY) {
  ...
  fit_out <- fit_dt[fit_dt$tag %in% c(intervention_names, paste0("mobility_delta_", INTERVENTION_PARAMS$MOB_VAR)) ]
  ...
  if (OUTPUT_OPTION$CONVERT_COEF & INTERVENTION_PARAMS$VAR_TYPE == "scale") {
    ...
    scale_factor <- setNames(c(scale_factor, 10), c(names(scale_factor), paste0("mobility_delta_", INTERVENTION_PARAMS$MOB_VAR)))
